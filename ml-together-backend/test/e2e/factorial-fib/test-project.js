...
async function initializeTasks(projectData) {
    ...
    const chunk = Math.floor(projectData.dataLength / projectData.chunkSize);
    let lowerBound = 0;
    let UpperBound = projectData.chunkSize - 1;

    for (let i = 0; i < chunk; i++) {
        const dataLocation = new URL(projectData.url);
...
const return_result = async (result) => { // Returns results to the server
    await client.message(JSON.stringify({ event: 'result', result }));
};
